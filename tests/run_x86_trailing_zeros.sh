...
"$compiler" tests/x86/x86_trailing_zeros.zag -o "$tmp/generic" --cpu=generic --no-zagd --no-analyze >/dev/null
run_expect "$tmp/generic"
if has_tzcnt "$tmp/generic"; then echo "generic binary contains forbidden TZCNT" >&2; exit 1; fi
"$compiler" tests/x86/x86_trailing_zeros.zag -o "$tmp/runtime" --cpu=runtime --no-zagd --no-analyze >/dev/null
run_expect "$tmp/runtime"
has_tzcnt "$tmp/runtime"
if grep -qm1 -E '^flags.*(^| )bmi1( |$)' /proc/cpuinfo; then
  "$compiler" tests/x86/x86_trailing_zeros.zag -o "$tmp/native" --cpu=native --no-zagd --no-analyze >/dev/null
