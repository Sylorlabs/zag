...
if "$ZNC" tests/i686/i686_literal.zag --target i686 -o "$tmp/a.out" --no-analyze >/dev/null; then ok "literal main emits"; else bad "literal main emits"; fi
...
if "$ZNC" tests/i686/i686_literal.zag --target i686 --emit-obj -o "$tmp/main.o" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_literal.zag --target i686 --emit-static -o "$tmp/libmain.a" --no-analyze >/dev/null &&
...
if ! "$ZNC" tests/i686/i686_reject_pointer.zag --target i686 -o "$tmp/bad" --no-analyze >"$tmp/reject.log" 2>&1 &&
...
if ! "$ZNC" tests/i686/i686_reject_type_mismatch.zag --target i686 -o "$tmp/type-mismatch" --no-analyze >"$tmp/type-mismatch.log" 2>&1 &&
...
if ! "$ZNC" tests/i686/i686_reject_arity.zag --target i686 -o "$tmp/arity" --no-analyze >"$tmp/arity.log" 2>&1 &&
...
if ! "$ZNC" tests/i686/i686_reject_duplicate.zag --target i686 -o "$tmp/duplicate" --no-analyze >"$tmp/duplicate.log" 2>&1 &&
...
if ! "$ZNC" tests/i686/i686_reject_effect.zag --target i686 -o "$tmp/effect" --no-analyze >"$tmp/effect.log" 2>&1 &&
...
if ! "$ZNC" tests/i686/i686_reject_fallthrough.zag --target i686 -o "$tmp/fallthrough" --no-analyze >"$tmp/fallthrough.log" 2>&1 &&
...
if "$ZNC" tests/i686/i686_return_paths.zag --target i686 -o "$tmp/return-paths" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_i32_ir.zag --target i686 -o "$tmp/ir" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_integer_arithmetic.zag --target i686 -o "$tmp/arithmetic" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_usize_semantics.zag --target i686 -o "$tmp/usize" --no-analyze >/dev/null; then
...
if ! "$ZNC" tests/i686/i686_reject_usize_literal_range.zag --target i686 -o "$tmp/bad-usize-range" --no-analyze >"$tmp/bad-usize-range.log" 2>&1 &&
...
if ! "$ZNC" tests/i686/i686_reject_usize_negative.zag --target i686 -o "$tmp/bad-usize-negative" --no-analyze >"$tmp/bad-usize-negative.log" 2>&1 &&
...
if ! "$ZNC" tests/i686/i686_reject_usize_implicit.zag --target i686 -o "$tmp/bad-usize-mixed" --no-analyze >"$tmp/bad-usize-mixed.log" 2>&1 &&
...
if "$ZNC" tests/i686/i686_divide_by_zero.zag --target i686 -o "$tmp/divzero" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_control_flow.zag --target i686 -o "$tmp/cf" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_calls.zag --target i686 -o "$tmp/calls" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_abi_pressure.zag --target i686 -o "$tmp/abi-pressure" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_f32.zag --target i686 -o "$tmp/f32" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_i64.zag --target i686 -o "$tmp/i64" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_i64_divide_by_zero.zag --target i686 -o "$tmp/i64-divzero" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_i64_remainder_by_zero.zag --target i686 -o "$tmp/u64-remzero" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_float.zag --target i686 -o "$tmp/float" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_pointer.zag --target i686 -o "$tmp/pointer" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_pointer_index.zag --target i686 -o "$tmp/pointer-index" --no-analyze >/dev/null; then
...
if ! "$ZNC" tests/i686/i686_reject_pointer_arithmetic.zag --target i686 -o "$tmp/bad-pointer-arithmetic" --no-analyze >"$tmp/bad-pointer-arithmetic.log" 2>&1 &&
...
if ! "$ZNC" tests/i686/i686_reject_pointer_integer_cast.zag --target i686 -o "$tmp/bad-pointer-cast" --no-analyze >"$tmp/bad-pointer-cast.log" 2>&1 &&
...
if ! "$ZNC" tests/i686/i686_reject_pointer_index_signed.zag --target i686 -o "$tmp/bad-pointer-index-signed" --no-analyze >"$tmp/bad-pointer-index-signed.log" 2>&1 &&
...
if ! "$ZNC" tests/i686/i686_reject_u64_range.zag --target i686 -o "$tmp/bad-u64-range" --no-analyze >"$tmp/bad-u64-range.log" 2>&1 &&
...
if ! "$ZNC" tests/i686/i686_reject_i64_range.zag --target i686 -o "$tmp/bad-i64-range" --no-analyze >"$tmp/bad-i64-range.log" 2>&1 &&
...
if ! "$ZNC" tests/i686/i686_reject_u64_negative.zag --target i686 -o "$tmp/bad-u64-negative" --no-analyze >"$tmp/bad-u64-negative.log" 2>&1 &&
...
if "$ZNC" tests/i686/i686_write.zag --target i686 -o "$tmp/write" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_large_frame.zag --target i686 -o "$tmp/large" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_write_error.zag --target i686 -o "$tmp/writeerr" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_syscall_lifecycle.zag --target i686 -o "$tmp/syscall-lifecycle" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_struct.zag --target i686 -o "$tmp/struct" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_print.zag --target i686 -o "$tmp/print" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_error_union.zag --target i686 -o "$tmp/errorunion" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_reject_try_non_error.zag --target i686 -o "$tmp/bad-try" --no-analyze >/dev/null 2>&1; then
...
if "$ZNC" tests/i686/i686_reject_catch_non_error.zag --target i686 -o "$tmp/bad-catch" --no-analyze >/dev/null 2>&1; then
...
if "$ZNC" tests/i686/i686_struct_arg.zag --target i686 -o "$tmp/structarg" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_struct_return.zag --target i686 -o "$tmp/structret" --no-analyze >/dev/null; then
...
if ! "$ZNC" tests/i686/i686_reject_struct_return.zag --target i686 -o "$tmp/badstructret" --no-analyze >"$tmp/badstructret.log" 2>&1 &&
...
if "$ZNC" tests/i686/i686_slice_abi.zag --target i686 -o "$tmp/slice" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_slice_index.zag --target i686 -o "$tmp/slice-index" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_slice_oob_negative.zag --target i686 -o "$tmp/slice-oob-negative" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_slice_oob_usize.zag --target i686 -o "$tmp/slice-oob-usize" --no-analyze >/dev/null; then
...
if ! "$ZNC" tests/i686/i686_reject_index_type.zag --target i686 -o "$tmp/bad-index-type" --no-analyze >"$tmp/bad-index-type.log" 2>&1 &&
...
if ! "$ZNC" tests/i686/i686_reject_slice_mutation.zag --target i686 -o "$tmp/badslice" --no-analyze >"$tmp/badslice.log" 2>&1 &&
...
if "$ZNC" tests/i686/i686_import_runtime.zag --target i686 -o "$tmp/import-runtime" --no-analyze >/dev/null; then
...
if "$ZNC" tests/i686/i686_alloc_failure.zag --target i686 -o "$tmp/alloc-failure" --no-analyze >/dev/null; then
