...
"$compiler" tests/x86_andn.zag -o "$tmp/generic" --cpu=generic --no-zagd --no-analyze >/dev/null
...
"$compiler" tests/x86_andn.zag -o "$tmp/runtime" --cpu=runtime --no-zagd --no-analyze >/dev/null
...
if grep -qm1 -E '^flags.*(^| )bmi1( |$)' /proc/cpuinfo; then
  "$compiler" tests/x86_andn.zag -o "$tmp/native" --cpu=native --no-zagd --no-analyze >/dev/null
