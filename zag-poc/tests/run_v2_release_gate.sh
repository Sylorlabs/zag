...
unsupported "effect adversarial suite" "opaque aggregate/computed indirect calls fail closed and distinct Atomic/FFI/Device/GPUHost effects propagate through helpers and function-value storage, but verified aggregate effect-row calls and a complete device capability model remain incomplete"
