...
"$compiler" tests/x86_popcount.zag -o "$tmp/generic" --cpu=generic --no-zagd --no-analyze >/dev/null
set +e
"$tmp/generic"
generic_rc=$?
set -e
...
if grep -qm1 -E '^flags.*(^| )popcnt( |$)' /proc/cpuinfo; then
    "$compiler" tests/x86_popcount.zag -o "$tmp/native" --cpu=native --no-zagd --no-analyze >/dev/null
    set +e
    "$tmp/native"
    native_rc=$?
    set -e
