...
| 7. CPU control | unsafe/target model | asm operand/clobber validation and machine-code tests | PARTIAL: validated x86 POPCNT, BMI1 ANDN, trailing-zero, byte-swap, leading-zero, baseline prefetch, and bounded unsafe SSE2 `i32x4` add/subtract/and/or/xor paths have machine-code/runtime evidence; vector types/ABI, broader SIMD, inline asm constraints, and full target-feature/effect contracts remain |
| 8. GPU runtime | typed device semantics | bounded Vulkan/OpenGL/OpenCL loader dispatch/readback with shared launch contracts; AMDGPU DRM discovery/GPUVM/CPU-memory preflight; direct CS submission, fence execution, and complete device typing remain | PARTIAL |
