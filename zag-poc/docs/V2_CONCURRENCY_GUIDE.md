...
The current v2 public atomic slice is intentionally three fixed-order
operations: `@atomicLoad64(ptr: *const/*mut i64) i64`,
`@atomicStore64(ptr: *mut i64, value: i64) void`, and
`@atomicExchange64(ptr: *mut i64, value: i64) i64`. They are callable only
...
types, CAS/fetch operations, selectable memory orders, fences with
...
`tests/run_v2_atomic_exchange.sh` proves load/store/exchange results and
inspects the emitted ELF for locked `xadd` and `xchg`; it also covers the
unsafe, pointer-mutability, and misalignment failure paths. A successful
