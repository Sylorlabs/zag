...
| Raw pointers, provenance, alignment, aliasing | `V2_MEMORY_MODEL.md` | typed qualifier recognition, named-origin ownership/borrow flow, bounded mutation-aware field-path provenance, and pre-lowering machine-control checks in `selfhost/typed.zag`; native pointer lowering | `tests/run_v2_edition.sh` executes `*const` construction/dereference, explicitly unwrapped nullable dereference, owned-return/release positives, and caller-provided aggregate pointer returns; `tests/run_v2_aggregate_provenance.sh` covers safe field/copy clearing and fail-closed paths beyond the cap | edition 2026 qualifier rejection, nullable dereference, cross-address-space casts, safe-scope dereference, const mutation, tested double/use-after-free and ownership/borrow violations, callee-frame address returns, and named aggregate pass/return/store escapes reject without artifacts | native x86-64 execution; `run_allocator_lifetime.sh` additionally proves stale dereference traps after a freed dedicated large mapping is unmapped | PARTIAL / named static origins, explicit contracts, selected control-flow joins, and 64-field-deep aggregate mutation/copy provenance; dynamic bounds/alignment, arbitrary heap graphs, callback/global lifetime proof, and general runtime use-after-free instrumentation remain |
