...
| Allocation, resize, free, custom allocators | `V2_MEMORY_MODEL.md`, `V2_ALLOCATOR_GUIDE.md` | `_zag_malloc`, `_zag_realloc`, `_zag_free` lowering with bounded large-map provenance and allocator telemetry; checked native `std/allocator.zag` provides fallible `SystemAllocator.allocate` and consuming `deallocate` over `{ptr, exact_capacity, alignment}` handles | `tests/run_allocator_lifetime.sh` executes allocation, reuse, resize, large-map accounting, and Script allocation witnesses; `tests/run_v2_edition.sh` executes a handle allocation/deallocation and traps forged-length and copied-released handles before free | rejects negative allocation/resize sizes, invalid/double free, stale realloc, raw-slice double free, bounded-registry exhaustion, forged SystemAllocator length, and copied released SystemAllocator handles | x86-64 native execution; large stale dereference traps at the unmapped-page boundary | PARTIAL / no generation or allocator identity (ABA remains), resize/zeroed/custom/arena/fixed-buffer allocators, or general dynamic lifetime system |
