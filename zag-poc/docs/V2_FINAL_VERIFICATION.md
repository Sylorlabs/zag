...
| Allocation, resize, free, custom allocators | `V2_MEMORY_MODEL.md` | `_zag_malloc`, `_zag_realloc`, `_zag_free` lowering with bounded large-map provenance and allocator telemetry | `tests/run_allocator_lifetime.sh` executes allocation, reuse, resize, large-map accounting, and Script allocation witnesses | rejects negative allocation/resize sizes, invalid/double free, stale realloc, raw-slice double free, and bounded-registry exhaustion | x86-64 native execution; large stale dereference traps at the unmapped-page boundary | PARTIAL / not a specified v2 allocator API or general dynamic lifetime system |
