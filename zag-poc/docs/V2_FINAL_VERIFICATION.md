...
| Raw pointers, provenance, alignment, aliasing | `V2_MEMORY_MODEL.md` | typed qualifier recognition, named-origin ownership/borrow flow, bounded mutation-aware field-path provenance, pre-lowering machine-control checks in `selfhost/typed.zag`, and `--safety=checked` null/alignment guards in native x86-64 lowering | `tests/run_v2_edition.sh` executes `*const` construction/dereference, explicitly unwrapped nullable dereference, checked valid 24-byte aggregate access, owned-return/release positives, and caller-provided aggregate pointer returns; `tests/run_v2_aggregate_provenance.sh` covers safe field/copy clearing and fail-closed paths beyond the cap | edition 2026 qualifier rejection, nullable dereference, checked null/misaligned dereference, cross-address-space casts, safe-scope dereference, const mutation, tested double/use-after-free and ownership/borrow violations, callee-frame address returns, callback capture, and named aggregate pass/return/store escapes reject without artifacts | native x86-64 execution; `run_allocator_lifetime.sh` additionally proves stale dereference traps after a freed dedicated large mapping is unmapped | PARTIAL / checked mode currently covers only null/alignment on native x86-64; dynamic bounds, arbitrary heap graphs, callback/global lifetime contracts, and general runtime use-after-free instrumentation remain |
