...
| Raw pointers, provenance, alignment, aliasing | `V2_MEMORY_MODEL.md` | typed qualifier recognition, named-origin ownership/borrow flow, bounded mutation-aware field-path provenance, scalar captured-callback ownership with `close`, root-only zero-initialized primitive scalar globals, pre-lowering machine-control checks in `selfhost/typed.zag`, and `--safety=checked` native x86-64 null/alignment plus tracked ordinary-allocation bounds/freed-region guards | `tests/run_v2_edition.sh` executes `*const` construction/dereference, checked valid 24-byte aggregate and tracked in-bounds heap access, owned-return/release positives, a returned scalar captured callback followed by `close`, scalar global read/write, and caller-provided aggregate pointer returns; `tests/run_v2_aggregate_provenance.sh` covers safe field/copy clearing and fail-closed paths beyond the cap | edition 2026 qualifier rejection, nullable dereference, checked null/misaligned dereference and tracked one-past heap access, cross-address-space casts, safe-scope dereference, const mutation, tested double/use-after-free and ownership/borrow violations, callee-frame address returns, pointer/aggregate callback capture, pointer/aggregate/nonzero-initialized globals, callback use after close, and named aggregate pass/return/store escapes reject without artifacts | native x86-64 execution; `run_allocator_lifetime.sh` additionally proves stale dereference traps after a freed dedicated large mapping is unmapped | PARTIAL / checked mode has bounded ordinary-allocation runtime metadata, scalar callback reclamation, and narrow scalar BSS globals, but arbitrary heap graphs, raw-slice/custom allocator tracking, ABA-resistant lifetime identity, pointer/aggregate global contracts, pointer/aggregate callback capture, and general runtime use-after-free instrumentation remain |
