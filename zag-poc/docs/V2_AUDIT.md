...
| Heap allocation | `cg_lower_new`, `_zag_malloc`, `_zag_realloc`, `_zag_free` paths; checked `std/allocator.zag` `SystemAllocator` | Raw allocation remains an incomplete v2 contract, but checked native `SystemAllocator` now exposes fallible allocate/zeroed/resize/deallocate handles with capacity, alignment, and generation validation. General ownership, debug allocation, and raw-allocation safety are still incomplete. |
